...
**Objective and Scope**:
System which allows the creator to create a new the URL from service in order to shorten it. Then the consumer uses this service to reach the creators URL and gets redirected to the actual URL. Reduces the need to use long URLs. Build it for internal use cases first, then expand to web scale in the next version. 
...
- If URL does not exist, show clear error 

**Non Functional Requirements**: 
- Assume the system maxes out at 10s URLs
- 100s of reqs/min 
- p95 Latency for redirection should be within the 10 ms expected by users for internal use cases
